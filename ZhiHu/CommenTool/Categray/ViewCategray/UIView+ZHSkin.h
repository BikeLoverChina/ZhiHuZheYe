 ...
 */
typedef void(^ZHClipedCallback)(UIImage *clipedImage);
 ...
 * 剪裁。下面是使用SDWebImage的一个例子：
 ...
 // 将剪裁后的图片记录下来，下次直接使用
 model.clipedImage = [image zh_clipToSize:weakSelf.imageView.bounds.size
 cornerRadius:12
 ...
 */
@property (nonatomic, assign) CGFloat zh_borderWidth;
/**
 *	当小于0时，不会添加边框。默认为0.仅对生成圆形图片和矩形图片有效
 */
@property (nonatomic, assign) CGFloat zh_pathWidth;
/**
 *	边框线的颜色，默认为[UIColor lightGrayColor]，仅对生成圆形图片和矩形图片有效
 */
@property (nonatomic, strong) UIColor *zh_borderColor;
/**
 *	Path颜色，默认为白色。仅对生成圆形图片和矩形图片有效
 */
@property (nonatomic, strong) UIColor *zh_pathColor;
 ...
 *  默认为NO
 */
@property (nonatomic, assign) BOOL zh_shouldRefreshCache;
 ...
 *
 *  @Example
 *  [cornerView3 zh_setImage:@"bimg8.jpg" cornerRadius:10 rectCorner:UIRectCornerTopLeft |UIRectCornerBottomRight isEqualScale:NO onCliped:^(UIImage *clipedImage) {
 ...
 *
 * @Example
 * 添加一个圆角：[view1 zh_addCorner:UIRectCornerTopLeft cornerRadius:10];
 */
 ...
 */
- (void)zh_addCorner:(UIRectCorner)corner
